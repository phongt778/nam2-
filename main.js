...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // expose helper functions so they can be called from console/tests
  try{ window.showImage = showImage; window.clearImage = clearImage; }catch(e){/* ignore */}
  ...
  // (optional) To programmatically set an avatar from an external URL or asset,
  // open DevTools Console and run one of the helpers exposed above, e.g.: 
  //   fetch('./assets/avatar.jpg').then(r=>r.blob()).then(b=>new Promise(r=>{const f=new FileReader();f.onload=()=>r(f.result);f.readAsDataURL(b);})).then(window.showImage)
});